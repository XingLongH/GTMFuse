...
class Fusion_dataset(Dataset):
    ...
    def __getitem__(self, index):
        if self.split=='train':
            ...
            image_inf = cv2.resize(image_inf, (256,256), interpolation=cv2.INTER_LINEAR)    
            image_vis = cv2.resize(image_vis, (256,256), interpolation=cv2.INTER_LINEAR)         

            # image_vis = (
            #     np.asarray(Image.fromarray(image_vis), dtype=np.float32) / 255.0
            # )
            image_vis = (
